...
description: >
  ...
  what shipped, what moved the needle, and what's next. Applies the ResearchXL / ICE-backed scoring
  model: Impact, Confidence, Ease, tempered by the PIE framework's Potential/Importance/Ease
  cross-check and a validity-threat gate to prevent shipping learnings from compromised tests.
...
Raw audit notes and a spreadsheet of tests are not stakeholder communication. This skill takes whatever CRO material you have — heuristic findings, experiment results, a running test log — and turns it into two clean artifacts: a scored **CRO Scorecard** stakeholders can act on, and a **Weekly Standup Digest** they'll actually read. The framework is ResearchXL / ICE, validity-gated to keep bad-data "wins" out of the record.
...
Step 3  Score opportunities   ──► ResearchXL / ICE × PIE cross-check
...
- Heuristic audit notes or screenshots (structured or freeform)
- Experiment results (win/loss/inconclusive + sample sizes, CVRs, confidence levels)
- Running test inventory (name, hypothesis, variant count, start date, current status)
- Analytics friction data (funnel drop-off %, scroll depth, rage-click coordinates)
...
- SRM (sample ratio mismatch) — if variant/control split deviates >5% from intended, mark result INVALID
- `(not set)` inflation above 5% in the relevant dimension
...
## The Scoring Framework (ResearchXL / ICE × PIE)
...
| **Confidence** (C) | Strength of evidence (quantitative > qualitative > heuristic) | A heuristic alone is low-C; a heatmap + session + analytics is high-C |
...
Cross-check with PIE (**Potential × Importance × Ease**): if PIE and ICE diverge by more than 2 ranks, flag the item for review — usually signals a high-traffic low-evidence tension worth surfacing to stakeholders.

Validity gate: any experiment result used to calibrate confidence must pass the Step 2 data-quality check first. Invalid results lower confidence to ≤3.
...
| # | Page / Surface | Finding source | ICE | Tier | Test status | Notes |
|---|---|---|---|---|---|---|
| 1 | [page] | [heuristic / heatmap / funnel data] | 8.2 | P1 | Running (wk 3) | SRM clear |
| 2 | ...
...
- **ICE scores rank, they don't decide.** Flag PIE divergences; don't pretend a formula replaces judgment.
- **Validity before learning.** SRM, sampling, or attribution issues make a test result inadmissible in the learning record — label it explicitly.
...
- Don't publish a "win" from a test with SRM or under the data-quality gate — mark it `Invalid` in the scorecard.
...
- ICE scores computed; PIE cross-check run; divergences flagged?
- SRM-flagged or invalid tests excluded from "what moved the needle" in the Digest?
