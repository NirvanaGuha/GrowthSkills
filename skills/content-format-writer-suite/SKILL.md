...
**Framework — Gartner Quadrant logic applied to prose:**
...
2. **Criteria table** — list 4–7 comparison dimensions relevant to the reader's job-to-be-done (not a feature dump); define each criterion briefly.
3. **Per-product section** — for each alternative: one-paragraph honest description, a filled criteria row, 1–2 limitations stated plainly.
4. **Head-to-head summary table** — criteria rows × products, cell values not raw checkmarks (use "Included in all plans" vs. "Yes").
5. **Who should choose X / Who should choose Y** — segment-specific verdicts; one or two sentence each. Earns trust; does not avoid the recommendation.
6. **Verdict + CTA** — one paragraph, brand's product mentioned where it fits, CTA from `cta-variant-generator` matching the awareness stage.

**Calibration rules:** do not trash competitors; cite limitations factually; any benchmark number not from the brand's own published data → `[verify]`. Length: 1,000–2,000 words typical (scales with number of alternatives).
...
**Framework — David Ogilvy's numbered-headline principle + inverted pyramid per item:**
1. **H1 + intro** (3–5 sentences max) — promise the list, state the selection criterion, name the intended reader. No lengthy preamble.
2. **Numbered items** — each item: `### N. [Action verb or noun phrase]` → 1–3 short paragraphs covering what it is, why it qualifies, and one concrete example or proof point. Items in decreasing-energy order is a myth — lead with the strongest to hook scanners.
3. **Quick-reference table** (optional for 10+ items) — name, category, one-line use case; scannable summary.
4. **Closing CTA** — one sentence + button copy from `cta-variant-generator`.

**Calibration rules:** every item must actually belong (no filler to hit a round number); if an item needs more than ~120 words it belongs in a how-to or comparison, not here. Aim for consistency of depth across items. Length: 800–1,500 words typical.
...
**Framework — Eugene Schwartz "before/after/bridge" scaled to narrative:**
1. **Hero summary** (above the fold) — customer name, their industry/size, the problem solved, and the headline result. Readers share this line; make it quotable.
...
3. **The decision** — why they chose this solution (not a puff paragraph; include the real reason — ease, price, integration, rep from a peer).
4. **Implementation** — what they actually did; timeline if available; any friction. Specific over vague.
...
7. **What made it work** — 2–3 factors the customer credits; keeps the case study from reading like a one-sided ad.
...
**Calibration rules:** do not write a case study from scratch without real customer input; if no confirmed data is available, write a `[PLACEHOLDER — needs customer data]` draft structure and note what must be verified before publish. Length: 600–1,200 words.
...
- For comparisons: balanced tone, head-to-head table present, segment-specific verdicts included?
- For listicles: every item earns its place; consistent depth; strong items first?
